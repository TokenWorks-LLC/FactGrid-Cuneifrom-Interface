      ...
      <div className="site-container grid gap-14 py-12 lg:grid-cols-[18rem_minmax(0,1fr)] lg:py-16">
        ...
        <div className="min-w-0">
          ...
          {tablet.editions.map((edition, index) => (
            <EditionSection edition={edition} key={edition.editionId ?? edition.reference?.url ?? index} qid={tablet.qid} />
          ))}
...
function EditionSection({ edition, qid }: { edition: TabletEdition; qid: string }) {
  ...
  const supportingSections = (revision?.sections ?? []).filter(
    (section) => !/^transcript|transliteration$/iu.test(section.heading),
  );
  ...
  return (
    <section className="border-b border-border py-9">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Badge variant={edition.status === "loaded" ? "secondary" : "outline"}>
              {edition.status === "loaded" ? "Available" : edition.status === "missing" ? "Missing page" : "Read only"}
            </Badge>
          </div>
          {edition.reference ? (
      ...
      {transcript ? (
        <div className="mt-7">
          <p className="font-mono text-xs tracking-[0.1em] text-muted-foreground uppercase">Transliteration</p>
          <pre className="mt-3 max-h-[42rem] overflow-auto border border-border bg-card p-5 font-mono text-sm leading-7 whitespace-pre-wrap sm:p-7">
            {wikitextToPlainText(transcript.displayText) || "The recognized transcript region is empty."}
          </pre>
          {!transcript.editable ? (
            <p className="mt-3 flex gap-2 text-sm leading-6 text-muted-foreground">
              ...
              {transcript.reason ?? "This source format is available for reading but is not safe for plain-text editing."}
            </p>
          ) : null}
        </div>
      ) : revision ? (
      ...
      {supportingSections.length > 0 ? (
        <div className="mt-8 space-y-7">
          {supportingSections.map((section, index) => {
            ...
            if (!text) return null;
            return (
              <details className="group border-t border-border pt-4" key={`${section.heading}-${index}`}>
                <summary className="focus-ring flex min-h-11 cursor-pointer list-none items-center justify-between gap-4 font-heading text-xl font-medium">
                  {section.heading}
                  <span aria-hidden="true" className="font-sans text-sm text-muted-foreground group-open:rotate-45">+</span>
                </summary>
                <div className="mt-3 max-w-[75ch] whitespace-pre-wrap text-[1.02rem] leading-8 text-muted-foreground">{text}</div>
              </details>
            );
          })}
        </div>
      ...
      {transcript?.editable && edition.editionId && historyUrl && revision ? (
        <EditionEditGate
